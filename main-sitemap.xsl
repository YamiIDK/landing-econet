			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - Econet Editora</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - Econet Editora</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="http://proyecto-econetlocal.local/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
